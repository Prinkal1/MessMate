...
function Suggestion() {
  const registerSuggestions = async (e) => {
    ...
    const response = await fetch("http://localhost:4000/pages/registerSuggestion", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify({student: student._id, title, description: desc}),
    });
  ...
  return (
    <div className="w-full h-screen flex flex-col gap-10 items-center  max-h-screen overflow-y-auto">
      <h1 className="text-white font-bold text-5xl mt-5 pb-[4rem] ">Suggestions</h1>
      <form
        method="POST"
