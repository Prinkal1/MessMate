...
function AdSuggestion() {
  ...
  const updateSuggestion = async (id) => {
    setLoader(true);
    const response = await fetch("http://localhost:4000/pages/updateSuggestion", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  return (
    <div className="w-full h-screen flex flex-col gap-3 items-center">
      <h1 className="text-white font-bold text-5xl pt-[3rem]">Suggestions</h1>
      <div className="bg-neutral-950 px-10 py-5 rounded-xl shadow-xl sm:w-[50%] sm:min-w-[450px] w-full mt-5 max-h-[25rem] h-[18rem] overflow-auto">
        <span className="text-white font-bold text-3xl pt-[3rem]">All Student's Suggestions</span>
        <ul role="list" className="divide-y divide-gray-700 text-white text-3xl pt-[2rem]">
