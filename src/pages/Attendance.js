...
function Attendance() {
  ...
  const getAttendance = async () => {
      ...
      const res = await fetch("http://localhost:4000/pages/getAttendance", {
        method: "POST",
        headers:{
  ...
  return (
    <div className="w-full h-screen flex flex-col gap-5 items-center mt-[6rem] max-h-screen overflow-y-auto pt-20 md:pt-0 ">
      <h1 className="text-white font-bold text-5xl">Attendance</h1>
      <ul className="flex gap-9 text-white text-2xl px-5 sm:p-0 text-center">
        <li>Total Days: {totalDays}</li>
        <li>Present Days: {totalDays - daysOff}</li>
        <li>Absent days: {daysOff}</li>
      </ul>
      <div className="flex gap-9 flex-wrap max-h-96 justify-center items-center">
        <Doughnut
          datasetIdKey="id"
          data={{
            labels,
        ...
        />
        <div className="flow-root bg-neutral-950 rounded-lg shadow-xl w-full mx-5 sm:m-0 sm:w-[30rem] p-5">
              ...
              <li className="py-3 sm:py-4" key={day.weekdate}>
                <div className="flex items-center space-x-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xl truncate text-gray-400">{day.present?"Present":"Absent"}</p>
                  </div>
