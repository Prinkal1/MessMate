...
function AdMessOff() {
  const getRequests = async () => {
    setProgress(30);
    const res = await fetch("http://localhost:4000/pages/listMessOff"
    //     , {
    //   method: "POST",
    //   headers: {
  ...
  const updateRequest = async (id, status) => {
    const res = await fetch("http://localhost:4000/pages/updateMessOff", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    const data = await res.json();
    if (data.success) {
      let user = newReqs.find((req) => req.id === id).user;
      toast.success(
        `Request from ${user.firstName} has been ${status}`,
        {
          position: "top-right",
          autoClose: 3000,
...
const reject = (id) => {
  ...
  return (
    <div className="w-full h-screen flex flex-col gap-3 items-center mt-[5rem]">
      ...
      <div className="bg-neutral-950 px-10 py-5 rounded-xl shadow-xl sm:w-[50%] sm:min-w-[450px] w-full mt-5 max-h-96 overflow-auto">
        ...
        <ul role="list" className="divide-y divide-gray-700 text-white">
          {newReqs.length === 0 ? (
            ...
            newReqs.map((req) => (
              <li
                className="group py-3 px-5 rounded sm:py-4 hover:bg-neutral-700 hover:shadow-2xl hover:scale-105 transition-all cursor-pointer"
                key={req.id}
              >
                <div className="flex items-center space-x-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-2xl font-medium truncate text-white">
                      {req.user.firstName} {req.user.lastName}
                    </p>
                    <p className="text-2xl truncate text-gray-400">
                  ...
                  <div className="flex gap-3">
                    ...
                    >
                      <svg
                        ...
                        strokeWidth={1.5}
                        stroke="currentColor"
                        className="w-9 h-9 hover:text-green-600 hover:scale-125
                      transition-all"
                      >
                        <path
                          strokeLinecap="round"
                    ...
                    >
                      <svg
                        ...
                        strokeWidth={1.5}
                        stroke="currentColor"
                        className="w-9 h-9 hover:text-red-500 hover:scale-125
                      transition-all"
                      >
                        <path
                          strokeLinecap="round"
